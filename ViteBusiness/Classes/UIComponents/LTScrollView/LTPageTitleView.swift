...
@objc public class LTPageTitleView: UIView {
    ...
    var sliderLineViewWidth: CGFloat = glt_sliderDefaultWidth {
        didSet {
            sliderLineView.frame.size.width = glt_sliderDefaultWidth
        }
    }
    ...
    private lazy var sliderScrollView: UIScrollView = {
        let sliderScrollView = UIScrollView(frame: CGRect(x: 0, y: 0, width: bounds.width, height: bounds.height - layout.pageBottomLineHeight))
        sliderScrollView.showsHorizontalScrollIndicator = false
        sliderScrollView.bounces = false
        return sliderScrollView
    }()
    
    lazy var pageBottomLineView: UIView = {
...
extension LTPageTitleView {
    ...
    private func setupButtonsLayout() {
        ...
        let subH = bounds.height - layout.bottomLineHeight - layout.pageBottomLineHeight
    ...
    private func setupTitleSelectIndex(_ btnSelectIndex: Int) {
        ...
        if layout.sliderWidth == glt_sliderDefaultWidth {
            if layout.isAverage {
                ...
                sliderLineViewWidth = glt_lineWidths[btnSelectIndex]
            }else {
                sliderLineView.frame.origin.x = nextButton.frame.origin.x
                sliderLineViewWidth = nextButton.frame.width
            }
        }else {
        ...
        if isClickScrollAnimation {
            ...
            if layout.sliderWidth == glt_sliderDefaultWidth {
                ...
                if layout.isAverage {
                    ...
                    sliderLineView.frame.origin.x = nextButton.frame.origin.x + adjustX
                    sliderLineViewWidth = glt_lineWidths[index]
                }else {
                    sliderLineView.frame.origin.x = nextButton.frame.origin.x
                    sliderLineViewWidth = nextButton.frame.width
                }
    ...
    private func setupLineViewX(offsetX: CGFloat) -> Bool {
        ...
        if layout.sliderWidth == glt_sliderDefaultWidth {
            ...
            if layout.isAverage {
                ...
                self.sliderLineViewWidth = glt_lineWidths[sourceIndex] + moveW * progress
                ...
                self.sliderLineViewWidth = currentButton.frame.width + moveW * progress
