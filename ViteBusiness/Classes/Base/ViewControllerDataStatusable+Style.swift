...
extension UIView {
    ...
    open static func  networkErrorAndshowAccountView(error: Error, showImage: Bool = true, retry: @escaping () -> Void) -> UIView {
        ...
        let button = UIButton(type: .system).then {
            ...
            $0.setTitleColor(UIColor(netHex: 0x007AFF), for: .normal)
            $0.setTitleColor(UIColor(netHex: 0x007AFF).highlighted, for: .highlighted)
            $0.setTitle(R.string.localizable.transactionListTransactionNetErrorAndShowAccount(), for: .normal)
        }
