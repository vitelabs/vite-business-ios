...
class EthViteExchangeViewController: BaseViewController {
    ...
    var balance = Balance(value: BigInt(0))
    ...
    let addressView = EthViteExchangeViteAddressView().then {
        $0.textLabel.text = HDWalletManager.instance.account?.address.description ?? ""
    }
    ...
    func bind() {
        ...
        exchangeButton.rx.tap.bind { [weak self] in
            ...
            let address = Address(string: self.addressView.textLabel.text ?? "")
            ...
            guard let amountString = self.amountView.textField.text, !amountString.isEmpty,
                let a = amountString.toBigInt(decimals: TokenInfo.viteERC20.decimals) else {
            ...
            Workflow.ethViteExchangeWithConfirm(viteAddress: address.description, amount: amount, gasPrice: Float(self.gasSliderView.value), completion: { [weak self] (r) in
                guard let `self` = self else { return }
                if case .success = r {
