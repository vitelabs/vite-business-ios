//
//  MiningStakingConfirmView.swift
//  ViteBusiness
//
...
public class MiningStakingConfirmView: BottomPopView {

    var balance = Amount(0)
    
    public init() {
        let confirmButton = UIButton(style: .blue, title: R.string.localizable.miningStakingPageConfirmButtonTitle())
        let totalView = ConfirmAmountView(type: .amount)
        let amountView = SendAmountView(amount: "", token: TokenInfo.BuildIn.vite.value)
        
        super.init(title: R.string.localizable.miningStakingPageConfirmTitle(), buttons: [confirmButton], superview: UIApplication.shared.keyWindow!)
        ...
        confirmButton.rx.tap.bind { [weak self] in
            ...
            guard let amountString = amountView.textField.text,
                ...
                let amount = amountString.toAmount(decimals: TokenInfo.BuildIn.vite.value.decimals) else {
            ...
            Workflow.dexStakeForMiningWithConfirm(account: HDWalletManager.instance.account!, amount: amount) { _ in }
            
            }.disposed(by: rx.disposeBag)

        
        totalView.titleLabel.text = R.string.localizable.miningStakingPageConfirmAvailableTitle()
        
        amountView.titleLabel.textColor = UIColor.init(netHex: 0x3E4A59, alpha: 0.7)
        amountView.textField.keyboardType = .decimalPad
        amountView.titleLabel.text = R.string.localizable.miningStakingPageConfirmAmountTitle()
        amountView.textField.placeholder = R.string.localizable.miningStakingPageConfirmAmountPlaceholder()
        
        let tipLabel = UILabel().then {
            $0.text = R.string.localizable.miningStakingPageConfirmTip()
            $0.numberOfLines = 0
        ...
        ViteBalanceInfoManager.instance.dexBalanceInfosDriver
            .drive(onNext: { [weak self] (map) in
                guard let `self` = self else { return }
                self.balance =  map[TokenInfo.BuildIn.vite.value.viteTokenId]?.available ?? Amount(0)
                totalView.set(text: self.balance.amountFullWithGroupSeparator(decimals: TokenInfo.BuildIn.vite.value.decimals) +  " " + TokenInfo.BuildIn.vite.value.symbol)
            }).disposed(by: rx.disposeBag)
        
        amountView.allButton.rx.tap.bind { [weak self] in
            guard let `self` = self else { return }
            amountView.textField.text = self.balance.amountFull(decimals: TokenInfo.BuildIn.vite.value.decimals)
