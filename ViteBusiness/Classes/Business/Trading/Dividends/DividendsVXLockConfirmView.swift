//
//  DividendsVXLockConfirmView.swift
//  ViteBusiness
//
...
public class DividendsVXLockConfirmView: BottomPopView {

    var balance = Amount(0)
    
    public init() {
        let confirmButton = UIButton(style: .blue, title: R.string.localizable.dividendsPageLockConfirmButtonTitle())
        let totalView = ConfirmAmountView(type: .amount)
        let amountView = SendAmountView(amount: "", token: TokenInfo.BuildIn.vx.value)
        
        super.init(title: R.string.localizable.dividendsPageLockConfirmTitle(), buttons: [confirmButton], superview: UIApplication.shared.keyWindow!)
        ...
        confirmButton.rx.tap.bind { [weak self] in
            ...
            guard amount >= "1".toAmount(decimals: TokenInfo.BuildIn.vx.value.decimals)! else {
                Toast.show(R.string.localizable.dividendsPageLockConfirmError())
                return
            }
            ...
            Workflow.dexLockVxForDividendWithConfirm(account: HDWalletManager.instance.account!, amount: amount) { _ in }
            
            }.disposed(by: rx.disposeBag)

        
        totalView.titleLabel.text = R.string.localizable.dividendsPageLockConfirmAmountTotal()
        
        amountView.titleLabel.textColor = UIColor.init(netHex: 0x3E4A59, alpha: 0.7)
        amountView.textField.keyboardType = .decimalPad
        amountView.titleLabel.text = R.string.localizable.dividendsPageLockConfirmAmount()
        amountView.textField.placeholder = R.string.localizable.dividendsPageLockConfirmPlaceholder()
        
        let tipLabel = UILabel().then {
            $0.text = R.string.localizable.dividendsPageLockConfirmTip()
            $0.numberOfLines = 0
        ...
        ViteBalanceInfoManager.instance.dexBalanceInfosDriver
            .drive(onNext: { [weak self] (map) in
                guard let `self` = self else { return }
                self.balance =  map[TokenInfo.BuildIn.vx.value.viteTokenId]?.available ?? Amount(0)
