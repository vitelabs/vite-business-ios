...
class f__IntroductionViewController: BaseViewController {
    ...
    let imageView = UIImageView(image: LocalizationService.sharedInstance.currentLanguage == .chinese ?
        R.image.intro_zh() : R.image.intro_en())

    let label = UILabel().then {
        $0.textColor = UIColor(netHex: 0x007AFF)
        $0.font = UIFont.systemFont(ofSize: 27, weight: .regular)
        $0.numberOfLines = 0
        $0.textAlignment = .center
        $0.text = R.string.localizable.introductionTipTitle()
    }
